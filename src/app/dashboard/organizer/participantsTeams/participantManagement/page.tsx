...
export default function ParticipantManagementPage() {
  ...
  async function handleFormSubmit(values: FormSchema) {
    ...
    const newParticipant = {
      name: values.playerInfo.name || false,
      email: values.playerInfo.email|| false,
      questions: values.questions,
      rules: values.rules,
      walletAddress: values.playerInfo.walletAddress|| false,
      paymentDetails: values.playerInfo.paymentDetails|| false,
      entryFeeDetails: values.playerInfo.entryFeeDetails|| false,
      phoneNumber: values.playerInfo.phoneNumber|| false,
        ...
        <div>
          <div className="mb-8">
            ...
            <p className="text-muted-foreground mb-4">
              Manage tournament participants, teams, and registrations.
            </p>
            {tournament?.participant != null ? (
              <UrlCard
                ...
                url={`${window.location.origin}/dashboard/participant/tournaments/exploreTournament/${tournament._id}`}
              />
            ) : (
              <Button variant="green"  onClick={() => setView("formSetup")}>Setup Participant Form</Button>
            )}
          </div>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-8">
              ...
              <div className="flex justify-between">
                ...
                <Button variant="green" type="submit">Save Participant Settings</Button>
              </div>
            </form>
          </Form>
        </div>
