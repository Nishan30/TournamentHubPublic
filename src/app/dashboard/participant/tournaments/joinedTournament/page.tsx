...
export default function TournamentsPage() {
  ...
  useEffect(() => {
    if (!user.user) {
      ...
      router.push(`/signin?redirect=${encodeURIComponent(currentPath)}?accountType=Participant`);
      return undefined;
    }
    ...
    const fetchTournaments = async () => {
      try {
        ...
        const data = await getTournamentsByEmail(user.user?.email as string);
        setTournaments(data);
      } catch (err) {
  ...
  return (
    <div className="min-h-screen flex">
      <aside className="w-1/7 border-r border-neutral-800">
        <SidebarLayoutParticipant activeItem="Explore Tournament" />
      </aside>
