...
import { Caption } from "@stellar/design-system";
import { Profile } from "@stellar/design-system";
import { Notification } from "@stellar/design-system";
import { StellarWalletsKit, WalletNetwork, ISupportedWallet,WalletType } from "stellar-wallets-kit";
...
export const SendPayment = (props: SendPaymentProps) => {
  ...
  const [tokenId, setTokenId] = useState(props.tokenAddress || "");
  const [tokenDecimals, setTokenDecimals] = React.useState(XLM_DECIMALS);
  const [paymentDestination, setPaymentDest] = useState(props.paymentDestination || "");
  const [sendAmount, setSendAmount] = useState(props.amount || "");
  ...
  return (
    <>
      <div className="Layout__inset account-badge-row">
        {/* Render Profile only if activePubKey is not null */}
        {activePubKey !== null && (
      ...
      <div className="Layout__inset layout">
        <div className="payment">
          <Card className="p-4"> {/* Added padding to the Card */}
            <Caption size="sm" addlClassName="step-count">
              Step {stepCount} of 8
            </Caption>
            {renderStep(stepCount)} {/* Render the appropriate step based on the stepCount */}
          </Card>
        </div>
